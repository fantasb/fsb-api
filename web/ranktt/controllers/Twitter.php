...
class Twitter extends ApiControllerAbstract {
	...
	private function _getAppToken(){
		$key = Ranktt::getConfig('twitterAppKey');
		$secret = Ranktt::getConfig('twitterAppSecret');
		$creds = base64_encode($this->encode_rfc3986($key).':'.$this->encode_rfc3986($secret));

		$r = Ranktt::curlPost('https://api.twitter.com/oauth2/token', array(
	...
	public function get(){
		...
		$r = Ranktt::curlGet($url, null, array(
