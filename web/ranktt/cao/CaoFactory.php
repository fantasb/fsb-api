	...
	*/
	...
	protected static function classNameToCaoClassName($className, $caoDriver=false){
		/* this is where the magic happens, e.g.
			className = ranktt\controllers\Products > key = ranktt\controllers\Products_VersionController > return self::$caoClassnameCache[$key] (\ranktt\cao\VersionController)
			className = ranktt\adapters\Products > key = ranktt\adapters\Products_Version > return self::$caoClassnameCache[$key] (\ranktt\cao\Version)
		*/
		return $className;
	}
